...
| 📊 **事件状态机** | New/Ongoing/Stale/Resolved/Dismissed 五种状态 |
...
│   │   ├── App.tsx          # 根组件（Tab 路由）
│   │   ├── pages/           # 页面组件
│   │   │   ├── Dashboard.tsx    # 事件中心化主页（系统状态、信号收件箱、每日简报）
...
│   │   │   ├── events/      # 事件组件（EventCard、SignalInbox、SystemStatus、DailyBrief、MarketSnapshot）
│   │   │   ├── signals/     # 信号组件（Heatmap、Card、TopSignals）
│   │   │   └── sources/     # 数据源面板组件
│   │   ├── hooks/           # React 钩子
