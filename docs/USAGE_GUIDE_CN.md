...
12. [质量门控与交易建议](#12-质量门控与交易建议)
13. [未解决问题追踪](#13-未解决问题追踪open-loops)
...
| 功能模块 | 说明 |
|---------|------|
| 🖥️ **事件中心化仪表盘** | Robinhood 风格，从 Ticker 视图转为事件驱动设计 |
| 📋 **信号收件箱** | 事件卡片展示关注度评分、4D 评分、证据摘要、交易建议 |
| 📊 **事件状态机** | New/Ongoing/Stale/Resolved/Dismissed 五种状态，支持自动转换 |
| ⚡ **事件操作** | 支持置顶、推迟、标记已解决、驳回等操作 |
| 🎯 **质量门控** | 可配置的交易建议生成阈值 |
| 📝 **未解决问题** | 追踪研究项目和未解答的问题 |
| 🔌 **FastAPI 后端** | 信号、数据源、报告、简报、问题追踪和设置的 REST API |
...
│   ├── local_down.sh        # 一键停止
│   ├── run_state_transitions.py  # 运行事件状态机
│   ├── state_manager.py     # 状态管理工具
│   ├── state_transitions.sh # 状态转换脚本
│   ├── verify_db.py         # 数据库验证
...
│       ├── brief_service.py     # 每日简报操作
│       ├── loop_service.py      # 未解决问题操作
│       ├── settings_service.py  # 质量门控设置
│       └── system_service.py    # 系统健康检查
├── frontend/                # React 仪表盘（Robinhood 风格，事件中心化设计）
│   ├── src/
│   │   ├── App.tsx          # 根组件（Tab 路由）
│   │   ├── pages/           # 页面组件
│   │   │   ├── Dashboard.tsx    # 事件中心化主页
│   │   │   ├── EventDetail.tsx  # 事件详情（证据时间线）
│   │   │   ├── Signals.tsx      # 原始信号诊断表格
│   │   │   ├── Sources.tsx      # 数据源状态面板
│   │   │   ├── Reports.tsx      # 历史报告归档
│   │   │   ├── Settings.tsx     # 质量门控设置
│   │   │   └── UsageGuide.tsx   # 交互式使用指南
│   │   ├── components/      # UI 组件
...
│   │   │   ├── signals/     # 信号组件
│   │   │   ├── sources/     # 数据源面板组件
│   │   │   └── common/      # 通用组件（Button、ControlGroup）
│   │   ├── hooks/           # React 钩子
│   │   │   ├── useSignals.ts    # 信号数据获取（5分钟自动刷新）
│   │   │   ├── useEvents.ts     # 事件数据获取和操作
│   │   │   ├── useSources.ts    # 数据源健康状态
│   │   │   ├── useDailyBrief.ts # 每日简报数据
│   │   │   ├── useOpenLoops.ts  # 未解决问题管理
│   │   │   └── useQualityGateSettings.ts  # 质量门控设置
│   │   └── api/             # API 客户端
│   ├── package.json
│   └── vite.config.ts
├── docs/
...
```yaml
# 数据源优先级:
#   1. Capitol Trades（免费，无需 API 密钥）
#   2. Quiver Quantitative（需要 QUIVER_API_KEY）
#   3. Finnhub（需要 FINNHUB_API_KEY）
congress:
  enabled: true
...
**可选 API 密钥配置（在 .env 中设置）：**

```bash
# Quiver Quantitative - 免费试用（使用代码 'TWITTER'）
# 获取: https://api.quiverquant.com
QUIVER_API_KEY=your-key-here

# Finnhub - 免费层（60次请求/分钟）
# 获取: https://finnhub.io
...
2. 可选：在 `.env` 中配置备用 API 密钥
   ...
   FINNHUB_API_KEY=your-key-here
   ```
3. 系统会自动尝试备用数据源（Capitol Trades → Quiver → Finnhub）
...
| `/api/events` | GET | 获取事件列表（支持状态/排序/分页） |
| `/api/events/{event_id}` | GET | 获取事件详情（包含观察数据） |
| `/api/events/{event_id}/actions` | POST | 事件操作（置顶/取消置顶/推迟/驳回/解决） |
| `/api/briefs` | GET | 获取每日简报列表（分页） |
| `/api/briefs/latest` | GET | 获取最新每日简报 |
| `/api/briefs/{date}` | GET | 获取特定日期简报（YYYY-MM-DD） |
| `/api/loops` | GET | 获取未解决问题列表（可按状态过滤） |
| `/api/loops/{loop_id}` | GET | 获取未解决问题详情（包含进度笔记） |
| `/api/loops` | POST | 创建新的未解决问题 |
| `/api/loops/{loop_id}` | PATCH | 更新问题状态/添加进度笔记 |
| `/api/loops/{loop_id}` | DELETE | 删除未解决问题 |
| `/api/settings/gates` | GET | 获取质量门控设置 |
| `/api/settings/gates` | PUT | 更新质量门控设置 |
| `/api/settings/gates` | DELETE | 重置质量门控设置为默认值 |
| `/api/system/status` | GET | 获取数据源健康状态 |
...
**Today 页面**（Robinhood 风格事件中心化设计）：

*系统状态头部（SystemStatus）*：
- 数据质量概览：源数量、正常/错误/陈旧状态
- 最后更新时间
- 快速操作按钮：刷新数据、生成简报、发送邮件

*信号收件箱（SignalInbox）*：
- 事件卡片列表，按关注度评分排序
- 置顶事件优先显示
- 事件状态过滤：活跃（New/Ongoing）、已解决、全部
- 每个事件显示：
  - 关注度评分（0-100）
  - 4D 评分药丸（A/C/F/Conf）
  - 关联资产标签
  - 证据摘要
  - 交易计划预览（高置信度事件）
- 事件操作：置顶/取消置顶、推迟 24h、标记已解决、驳回

*事件状态机*：
| 状态 | 说明 | 颜色 |
|------|------|------|
| `new` | 新事件，首次出现 | 蓝色 |
| `ongoing` | 进行中（1小时后自动转换），持续跟踪 | 黄色 |
| `stale` | 过期，超过 72h 未更新（自动转换） | 灰色 |
...
*未解决问题（OpenLoops）*：
- 追踪需要进一步研究的问题
- 状态：open → in_progress → resolved | stale
- 进度笔记用于记录研究进展
- 可关联到具体事件

**EventDetail 页面**（事件详情）：
- 完整的 4D 评分分解（ScoreBreakdown）
- 证据时间线（EvidenceTimeline）
- 关联资产指标（AssetMetrics）
- 关键事实高亮（FactSpotlight）
- 操作面板（ActionPanel）
...
**Sources 页面**：
- 国会交易面板（CongressPanel）
- 对冲基金面板（HedgeFundPanel）
- 新闻面板（NewsPanel）
- Polymarket 面板（PolymarketPanel）
...
- min_sources: 最小数据源数量
- min_anomaly: 最小异常评分（0-100）
- min_catalyst: 最小催化剂评分（0-100）
- require_invalidation: 是否需要失效条件
- 一键重置为默认值
...
质量门控（Quality Gate）是用于评估事件是否达到生成交易建议标准的机制。

**默认阈值：**

| 阈值 | 默认值 | 说明 |
|------|--------|------|
| `min_confidence` | 70 | 最小置信度评分（0-100） |
| `min_sources` | 2 | 最小数据源数量 |
| `min_anomaly` | 50 | 最小异常评分（0-100） |
| `min_catalyst` | 40 | 最小催化剂评分（0-100） |
| `require_invalidation` | true | 是否需要失效条件 |

### 12.2 评估流程

```
事件 → QualityGate.evaluate() → QualityGateEvaluation
                                    ↓
                         ┌──────────┴──────────┐
                         ↓                     ↓
                    通过所有门控            未通过门控
                         ↓                     ↓
                    TradeIdea            ResearchPlan
                  (交易建议)              (研究计划)
```
...
| `entry_zone` | 入场区间，如 "$150-155" |
| `target` | 目标价，如 "$175" |
| `stop_loss` | 止损位，如 "$140" |
| `invalidation` | 失效条件 |
| `time_horizon` | 时间周期：intraday / swing / position / investment |
| `confidence_level` | 置信度（0-100） |
| `rationale` | 交易逻辑说明 |
| `key_catalysts` | 关键催化剂列表 |
| `risk_factors` | 风险因素列表 |

### 12.4 ResearchPlan（研究计划）

事件未通过质量门控时生成：

| 字段 | 说明 |
|------|------|
| `questions_to_verify` | 需要验证的问题列表 |
| `evidence_to_watch` | 需要观察的证据 |
| `next_check_date` | 下次检查日期 |
| `current_score` | 当前门控评分 |
| `gaps_identified` | 已识别的差距 |
...
```bash
# 获取当前设置
curl http://localhost:8002/api/settings/gates

# 更新设置
curl -X PUT http://localhost:8002/api/settings/gates \
  -H "Content-Type: application/json" \
  -d '{"min_confidence": 75, "min_sources": 3}'

# 重置为默认值
curl -X DELETE http://localhost:8002/api/settings/gates
```

**通过 Web 界面配置：**

访问 http://localhost:5173/settings 页面调整阈值。

---

## 13. 未解决问题追踪（Open Loops）

### 13.1 概述

未解决问题（Open Loops）用于追踪需要进一步研究或监控的问题。
...
```bash
# 获取问题列表
curl "http://localhost:8002/api/loops?status=open"

# 创建新问题
curl -X POST http://localhost:8002/api/loops \
  -H "Content-Type: application/json" \
  -d '{"question": "NVDA 财报后走势如何？", "event_id": "uuid-here"}'

# 更新问题状态
curl -X PATCH http://localhost:8002/api/loops/{loop_id} \
  -H "Content-Type: application/json" \
  -d '{"status": "in_progress", "progress_note": "正在分析期权数据"}'

# 删除问题
curl -X DELETE http://localhost:8002/api/loops/{loop_id}
```
...
pytest tests/test_daily_brief_generator.py
pytest tests/test_loops_api.py
